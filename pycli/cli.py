...
from pycli import (
    pycli, __app_name__, __version__, ERRORS, config, database
)
...
def get_todoer() -> pycli.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please run "pycli init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
        return pycli.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please run "pycli init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
